    ...
    switch (op) {
      ...
      case 'send-email-modal':
        ...
        if (this.transaction.company && this.transaction.company.emails) {
         ...
         let labelPrints = this.transaction.type.name;
    
        if (this.transaction.type.labelPrint) {
          labelPrints = this.transaction.type.labelPrint;
        }
        modalRef.componentInstance.subject = `${labelPrints} ${this.padNumber(
...
`        }
  ...
  async finish() {
    try {
      ...
      if(this.transaction.type.defectEmailTemplate !== null && this.transaction.type.requestEmailTemplate){
