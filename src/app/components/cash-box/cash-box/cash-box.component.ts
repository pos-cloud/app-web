...
export class CashBoxComponent implements OnInit {
  ...
  private ensureEmployee(): Promise<Employee | null> {
    if (this.identity?.employee) {
      return Promise.resolve(this.identity.employee);
    }
    return new Promise((resolve) => {
      const modalRef = this._modalService.open(SelectEmployeeComponent);
      modalRef.componentInstance.requireLogin = false;
      modalRef.componentInstance.op = 'select-employee';
      modalRef.result.then(
        (result) => resolve(result?.employee ?? null),
        () => resolve(null)
      );
    });
  }
    ...
    if (this.cashBox && this.cashBox._id) {
      if (this.cashBox.state === CashBoxState.Closed) {
        ...
        await this.getTransactions(query).then(async (transactions) => {
          if (transactions) {
            this.showMessage(
              'No puede cerrar la caja la transaccion : ' +
                transactions[0].type.name +
                '  ' +
                transactions[0].origin +
                '-' +
  ...
  async addTransaction() {
    ...
    if (this.movementsOfCashes.length > 0) {
      ...
      await this.saveTransaction().then(async (transaction) => {
        if (transaction) {
          this.transaction = transaction;
          for (let movementOfCash of this.movementsOfCashes) {
            movementOfCash.transaction = this.transaction;
          }
          await this.saveMovementsOfCashes().then(async (movementsOfCashes) => {
