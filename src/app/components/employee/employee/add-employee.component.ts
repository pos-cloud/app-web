      ...
      switchMap(async (term) => {
        let match: {} =
          term && term !== ''
            ...
            : {};
        return await this.getAllEmployeeTypes(match).then((result) => {
          return result;
        });
      }),
      tap(() => (this.loading = false))
    );
  public formatterEmployeeTypes = (x: EmployeeType) => {
    ...
    this._employeeService.saveEmployee(this.employee).subscribe(
      (result) => {
        if (!result.employee) {
          ...
          this.employee = result.employee;
          this.showMessage(
            'El empleado se ha añadido con éxito.',
            'success',
            false
          );
          this.employee = new Employee();
