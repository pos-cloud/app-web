...
export class BranchComponent implements OnInit {
  ...
  public setValueForm(): void {
    ...
    if (!this.branch.number) {
      this.branch.number = 0;
    }
    if (!this.branch.name) {
      this.branch.name = '';
    }
    if (!this.branch.default) {
      this.branch.default = false;
    }
  ...
  public buildForm(): void {
    this.branchForm = this._fb.group({
      _id: [this.branch._id, []],
      number: [this.branch.number, [Validators.required]],
      name: [this.branch.name, [Validators.required]],
      default: [this.branch.default, [Validators.required]],
      image: [this.branch.image, []],
    });

    this.branchForm.valueChanges.subscribe((data) => this.onValueChanged(data));
    this.onValueChanged();
  }
        ...
        (result) => {
          if (!result.branch) {
            ...
            if (this.filesToUpload) {
              this._branchService
                .makeFileRequest(result.branch, this.filesToUpload)
                .then(
                  (result) => {
                    this.branch.image = result['filename'];
                    this.showMessage(
                      'La sucursal se ha añadido con éxito.',
                      'success',
                      false
                    );
                    this.loading = false;
  ...
  public isValid(): Promise<boolean> {
    return new Promise((resolve, reject) => {
      if (
        ...
        this.branch.default !== null
      ) {
        for (const element of this.branches) {
          if (
            this.operation === 'add' &&
            this.branch.default &&
            element.default === this.branch.default
          ) {
            this.showMessage(
              'Solo puede existir una sucursal por defecto.',
              'danger',
              true
            );
            resolve(false);
