...
export class AddIdentificationTypeComponent implements OnInit {
  ...
  public buildForm(): void {
    this.identificationTypeForm = this._fb.group({
      ...
      name: [this.identificationType.name, [Validators.required]],
    });

    this.identificationTypeForm.valueChanges.subscribe((data) =>
  ...
  public onValueChanged(data?: any): void {
    if (!this.identificationTypeForm) {
      return;
    }
    ...
    this._identificationTypeService.getIdentificationTypes(query).subscribe(
      (result) => {
        if (!result.identificationTypes) {
          ...
          this.loading = false;
          try {
            this.identificationType.code = (
              parseInt(result.identificationTypes[0].code) + 1
            ).toString();
            this.setValuesForm();
          } catch (e) {}
        }
  ...
  public getIdentificationType(): void {
    ...
    this._identificationTypeService
      .getIdentificationType(this.identificationTypeId)
      .subscribe(
        (result) => {
  ...
  public setValuesForm(): void {
    if (!this.identificationType._id) {
      this.identificationType._id = '';
    }
    if (!this.identificationType.code) {
      this.identificationType.code = '1';
    }
    if (!this.identificationType.name) {
      this.identificationType.name = '';
    }
  ...
  public saveIdentificationType(): void {
    ...
    this._identificationTypeService
      .saveIdentificationType(this.identificationType)
      .subscribe(
        (result) => {
          if (!result.identificationType) {
            if (result.message && result.message !== '')
            ...
            this.identificationType = result.identificationType;
            this.showMessage(
              'El tipo de identificación se ha añadido con éxito.',
              'success',
              true
            );
            this.identificationType = new IdentificationType();
  ...
  public updateIdentificationType(): void {
    ...
    this._identificationTypeService
      .updateIdentificationType(this.identificationType)
      .subscribe(
        (result) => {
          if (!result.identificationType) {
            if (result.message && result.message !== '')
            ...
            this.identificationType = result.identificationType;
            this.showMessage(
              'El tipo de identificación se ha actualizado con éxito.',
              'success',
              true
            );
          }
  ...
  public deleteIdentificationType(): void {
    ...
    this._identificationTypeService
      .deleteIdentificationType(this.identificationType._id)
      .subscribe(
        (result) => {
