...
export class CurrencyComponent implements OnInit {
  ...
  public buildForm(): void {
    this.currencyForm = this._fb.group({
      ...
      sign: [this.currency.sign, [Validators.required]],
      name: [this.currency.name, [Validators.required]],
      quotation: [this.currency.quotation, [Validators.required]],
    });
    ...
    this._currencyService.getCurrencies(query).subscribe(
      (result) => {
        if (!result.currencies) {
          ...
          try {
            this.currency.code = (
              parseInt(result.currencies[0].code) + 1
            ).toString();
            this.setValuesForm();
          } catch (e) {}
        }
    ...
    this._currencyService.saveCurrency(this.currency).subscribe(
      (result) => {
        if (!result.currency) {
          ...
          this.currency = result.currency;
          this.showMessage(
            'La moneda se ha añadido con éxito.',
            'success',
            true
          );
          this.currency = new Currency();
    ...
    this._currencyService.updateCurrency(this.currency).subscribe(
      (result) => {
        if (!result.currency) {
          ...
          this.currency = result.currency;
          this.showMessage(
            'La moneda se ha actualizado con éxito.',
            'success',
            true
          );
        }
