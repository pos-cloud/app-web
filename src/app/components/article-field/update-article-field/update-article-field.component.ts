...
export class UpdateArticleFieldComponent implements OnInit {
  ...
  public setValueForm(): void {
    if (!this.articleField._id) {
      this.articleField._id = '';
    }
    if (!this.articleField.order) {
      this.articleField.order = 1;
    }
    if (!this.articleField.name) {
      this.articleField.name = '';
    }
    if (!this.articleField.datatype) {
      this.articleField.datatype = ArticleFieldType.Percentage;
    }
    if (this.articleField.modify === undefined) {
      this.articleField.modify = false;
    }
    if (this.articleField.modifyVAT === undefined) {
      this.articleField.modifyVAT = false;
    }
    if (this.articleField.discriminateVAT === undefined) {
      this.articleField.discriminateVAT = false;
    }
    if (this.articleField.ecommerceEnabled === undefined) {
      this.articleField.ecommerceEnabled = false;
    }
    ...
    this._articleFieldService.updateArticleField(this.articleField).subscribe(
      (result) => {
        if (!result.articleField) {
          ...
          this.showMessage(
            'El campo de producto se ha actualizado con éxito.',
            'success',
            false
          );
        }
