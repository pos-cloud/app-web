...
export class VATConditionComponent implements OnInit {
  ...
  public buildForm(): void {
    ...
    this.vatConditionForm.valueChanges.subscribe((data) =>
  ...
  public setValuesForm(): void {
    if (!this.vatCondition._id) {
      this.vatCondition._id = '';
    }
    if (!this.vatCondition.code) {
      this.vatCondition.code = 5;
    }
    if (!this.vatCondition.description) {
      this.vatCondition.description = '';
    }
    if (!this.vatCondition.transactionLetter) {
      this.vatCondition.transactionLetter = 'C';
    }
    if (!this.vatCondition.discriminate) {
      this.vatCondition.discriminate = false;
    }
    if (!this.vatCondition.observation) {
      this.vatCondition.observation = '';
    }
    ...
    this._vatConditionService.saveVATCondition(this.vatCondition).subscribe(
      (result) => {
        if (!result.vatCondition) {
          ...
          if (Config.country) {
            this.showMessage(
              'La condición de IVA se ha añadido con éxito.',
              'success',
              false
            );
          } else {
            this.showMessage(
              'El régimen fiscal se ha añadido con éxito.',
              'success',
              false
            );
          }
    ...
    this._vatConditionService.updateVATCondition(this.vatCondition).subscribe(
      (result) => {
        if (!result.vatCondition) {
          ...
          if (Config.country) {
            this.showMessage(
              'La condición de IVA se ha actualizado con éxito.',
              'success',
              false
            );
          } else {
            this.showMessage(
              'La régimen fiscal se ha actualizado con éxito.',
              'success',
              false
            );
          }
  ...
  public deleteVATCondition(): void {
    ...
    this._vatConditionService
      .deleteVATCondition(this.vatCondition._id)
      .subscribe(
        (result) => {
