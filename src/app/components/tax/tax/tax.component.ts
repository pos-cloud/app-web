...
export class TaxComponent implements OnInit {
  ...
  public formatterPrinters = (x: Printer) => {
    return x.name;
  };
  ...
  async ngOnInit() {
    ...
    this.buildForm();
    if (this.taxId) {
      await this.getTax(this.taxId).then((tax) => {
        if (tax) {
          this.tax = tax;
          this.setValuesForm();
        }
      ...
      await this.getTaxes('sort="code":-1&limit=1').then((taxes) => {
        if (taxes) {
          try {
  ...
  public buildForm(): void {
    this.taxForm = this._fb.group({
      _id: [this.tax._id, []],
      code: [this.tax.code, [Validators.required]],
      name: [this.tax.name, [Validators.required]],
      taxBase: [this.tax.taxBase, []],
      percentage: [this.tax.percentage, []],
      amount: [this.tax.amount, []],
      classification: [this.tax.classification, [Validators.required]],
      type: [this.tax.type, []],
      lastNumber: [this.tax.lastNumber, []],
      debitAccount: [this.tax.debitAccount, []],
      creditAccount: [this.tax.creditAccount, []],
      printer: [this.tax.printer, []],
    });

    this.taxForm.valueChanges.subscribe((data) => this.onValueChanged(data));
  ...
  public setValuesForm() {
    ...
    if (!this.tax.printer) this.tax.printer = null;
  ...
  public getAllPrinters(match: {}): Promise<Printer[]> {
