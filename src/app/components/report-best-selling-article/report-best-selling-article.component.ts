...
export class ReportBestSellingArticleComponent implements OnInit {
  ...
  async openModal(op: string, item: any[]) {
    ...
    switch (op) {
      case 'view':
        window.open(`/#/admin/articles/view/${item['article']._id}`, '_blank');
        break;
      default:
    }
  }
  ...
  public getItems(): void {
    ...
    let match = {
      'transaction.endDate': {
        $gte: {
          $date: this.startDate + 'T' + this.startTime + ':00' + timezone,
        },
    ...
    if (this.branchSelectedId) {
      match['transaction.branchDestination._id'] = {
        $oid: this.branchSelectedId,
      };
    }
    ...
    this.subscription.add(
      this._movementOfArticleService.getFullQuery(fullquery).subscribe(
        (result) => {
