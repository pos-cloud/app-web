...
export class PointOfSaleComponent implements OnInit {
  ...
  async refresh() {
    ...
    if (!this.transaction && this.transactionTypeId && this.transactionTypeId !== '') {
      ...
      this.getTransactionTypes(match).then((transactionTypes) => {
        if (transactionTypes) {
      ...
      if (this.posType === 'resto') {
        ...
        await this.getTransactionsV2(query).then((transactions) => {
          this.hideMessage();
  ...
  public async initTransactionByType(op: string, openPending: boolean = false) {
    ...
    await this.getTransactionTypes(match).then(async (transactionTypes) => {
      if (transactionTypes && transactionTypes.length > 0) {
        if (openPending) {
          ...
          await this.getTransactions(query).then((transactions) => {
    ...
    return new Promise<boolean>(async (resolve, reject) => {
      if (this.transaction.origin === 0) {
        ...
        await this.getOrigins({
          branch: { $oid: this.transaction.branchDestination._id },
          operationType: { $ne: 'D' },
        }).then((origins) => {
  ...
  async openModal(op: string, state: TransactionState = TransactionState.Closed) {
    ...
    switch (op) {
      ...
      case 'charge':
        ...
        break;
      case 'print':
        modalRef = this._modalService.open(FinishTransactionDialogComponent, {
          size: 'md',
          backdrop: 'static',
        });
        modalRef.componentInstance.transaction = this.transaction;
        ...
        modalRef.result.then(
          async (result) => {
            if (result && result.origin && result.destination) {
              let depositOrigin = await this.getDeposits({
                _id: { $oid: result.origin },
                operationType: { $ne: 'D' },
              });
              this.transaction.depositOrigin = depositOrigin[0];
              let branchO = await this.getBranches({
              ...
              let branchD = await this.getBranches({
  ...
  public getTransactionsV2(match: {}): Promise<Transaction[]> {
    return new Promise<Transaction[]>((resolve, reject) => {
      ...
      if (
        ...
        this.posType === 'mercadolibre' ||
        this.posType === 'woocommerce' ||
  ...
  async changeStateOfTransaction(transaction: Transaction, state: TransactionState) {
    ...
    if (this.transaction && !this.transaction.tiendaNubeId) {
      ...
      if (this.transaction.state === TransactionState.Closed) {
        ...
        await this.updateTransaction(this.transaction).then((transaction) => {
          if (transaction) {
            this.transaction = transaction;
            if (print) {
              this.refresh();
