...
export class PrintPriceListComponent implements OnInit {
  ...
  public buildForm(): void {
    ...
    this.printPriceListForm.valueChanges.subscribe((data) =>
  ...
  public getArticles(): void {
    ...
    if (
      this.printPriceListForm.value.articleField &&
      this.printPriceListForm.value.articleFieldValue
    ) {
  ...
  async printPriceListWithImagen() {
    ...
    this.centerText(
      ...
      row,
      'LISTA DE PRECIOS AL ' +
        this.dateFormat.transform(new Date(), 'DD/MM/YYYY')
    );
    ...
    if (this.articles && this.articles.length > 0) {
      for (let article of this.articles) {
        ...
        if (
          ...
          (await this.getPicture(article.picture))
        ) {
          try {
        ...
        if (this.printPriceListForm.value.priceList) {
          let priceList = await this.getPriceList(
            this.printPriceListForm.value.priceList
          );
          if (priceList) {
        ...
        if (article.otherFields && article.otherFields.length > 0) {
          for (let fields of article.otherFields) {
            ...
            this.doc.text(
              95,
              row,
              fields.articleField.name + ': ' + fields.value
            );
          }
        }
        row += 5;
        ...
        this.doc.text(
          95,
          row + 15,
          '$' + this.roundNumber.transform(article.salePrice).toString()
        );
            ...
            for (let value of variant['value']) {
              ...
              if (stock) {
                this.doc.text(
                  150,
                  rowAux,
                  value['description'] + ' /STK: ' + stock.toString()
                );
              } else {
  ...
  async printPriceListWithoutImagen() {
    ...
    if (this.articles && this.articles.length > 0) {
      for (let index = 0; index < this.articles.length; index++) {
        ...
        if (
          this.articles[index].make &&
          this.articles[index].make.description
        ) {
          this.doc.text(
            this.articles[index].make.description.slice(0, 18),
            100,
            row
          );
        }
        if (
          this.articles[index].category &&
          this.articles[index].category.description
        ) {
          this.doc.text(
            this.articles[index].category.description.slice(0, 18),
            145,
            row
          );
        }
            ...
            priceList.rules.forEach((rule) => {
              if (rule) {
                if (
                  ...
                  rule.make &&
                  this.articles[index].make &&
                  rule.category._id === this.articles[index].category._id &&
                  rule.make._id === this.articles[index].make._id
                ) {
                  increasePrice = rule.percentage + priceList.percentage;
                }
                if (
            ...
            priceList.exceptions.forEach((exception) => {
              if (exception) {
                if (
                  this.articles[index] &&
                  exception.article &&
                  exception.article._id === this.articles[index]._id
                ) {
                  increasePrice = exception.percentage;
                }
              }
        ...
        if (increasePrice != 0) {
          this.articles[index].salePrice =
            this.articles[index].salePrice +
            (this.articles[index].salePrice * increasePrice) / 100;
        }

        if (
          this.articles[index].currency &&
        ...
        this.doc.text(
          ...
          '$' +
            this.roundNumber
              .transform(this.articles[index].salePrice)
              .toString()
        );
      ...
      this._articleStockService
        ...
        .subscribe(
          (result) => {
            if (
              result &&
              result.articleStocks &&
              result.articleStocks.length > 0
            ) {
              let stock = 0;
