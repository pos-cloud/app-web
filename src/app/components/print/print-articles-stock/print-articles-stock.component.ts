...
export class PrintArticlesStockComponent implements OnInit {
  ...
  public toPrintInventario(articleStocks: ArticleStock[]): void {
    ...
    row += 5;
    if (
      this.config['companyIdentificationType'] &&
      this.config['companyIdentificationValue']
    ) {
      this.doc.text(
        this.config['companyIdentificationType']['name'] + ':',
        margin,
        row
      );
    ...
    this.doc.setFont('', 'bold');
    this.doc.text(
      'INVENTARIO AL ' + this.dateFormat.transform(new Date(), 'DD/MM/YYYY'),
      80,
      row
    );
    ...
    if (articleStocks && articleStocks.length > 0) {
      for (let articleStock of articleStocks) {
        ...
        if (
          articleStock.article.make &&
          articleStock.article.make.description
        ) {
          this.doc.text(
            articleStock.article.make.description.slice(0, 18),
            110,
            row
          );
        }
        if (
          articleStock.article.category &&
          articleStock.article.category.description
        ) {
          this.doc.text(
            articleStock.article.category.description.slice(0, 10),
            165,
            row
          );
        }
        if (articleStock.realStock) {
          this.doc.text(
            this.roundNumber.transform(articleStock.realStock).toString(),
            195,
            row
          );
        }
