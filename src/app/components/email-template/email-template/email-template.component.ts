...
export class EmailTemplateComponent implements OnInit {
  ...
  public getEmailTemplate() {
    ...
    this._emailTemplateService
      .getEmailTemplates(project, match, {}, {}, 1, 0)
      .subscribe(
        (result) => {
          if (!result.emailTemplates) {
            if (result.message && result.message !== '')
              this.showMessage(result.message, 'susses', true);
          } else {
            this.hideMessage();
  ...
  public setValueForm(): void {
    if (!this.emailTemplate._id) {
      this.emailTemplate._id = '';
    }
    if (!this.emailTemplate.name) {
      this.emailTemplate.name = '';
    }
    if (!this.emailTemplate.design) {
      this.emailTemplate.design = '';
    }
      ...
      .subscribe(
        (result) => {
          if (!result.emailTemplate) {
            ...
            this.loading = false;
            this.showMessage(
              'El template se ha actualizado con éxito.',
              'success',
              false
            );
          }
    ...
    this._emailTemplateService.saveEmailTemplate(this.emailTemplate).subscribe(
      (result) => {
        if (!result.emailTemplate) {
          ...
          this.loading = false;
          this.showMessage(
            'El template se ha añadido con éxito.',
            'success',
            false
          );
          this.emailTemplate = new EmailTemplate();
  ...
  public deleteEmailTemplate() {
    ...
    this._emailTemplateService
      .deleteEmailTemplate(this.emailTemplate._id)
      .subscribe(
        (result) => {
          this.loading = false;
          if (!result.emailTemplate) {
            if (result.message && result.message !== '') {
              this.showMessage(result.message, 'danger', true);
            }
          } else {
