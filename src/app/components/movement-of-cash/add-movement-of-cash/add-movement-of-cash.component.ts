...
export class AddMovementOfCashComponent implements OnInit {
      ...
      if (this.movementOfCashForm.value.amountToPay.toString().length > 1) {
        this.movementOfCashForm.value.amountToPay = parseFloat(
          this.movementOfCashForm.value.amountToPay
            ...
            .slice(
              0,
              this.movementOfCashForm.value.amountToPay.toString().length - 1
            )
        );
      } else {
        this.movementOfCashForm.value.amountToPay = 0;
  ...
  buildForm(): void {
    this.movementOfCashForm = this._fb.group({
      ...
      transactionAmount: [
        parseFloat(this.roundNumber.transform(this.transactionAmount)).toFixed(
          2
        ),
        [Validators.required],
      ],
      ...
      percentageAdministrativeExpense: [
    ...
    this.movementOfCashForm.valueChanges.subscribe((data) =>
  ...
  setValuesForm(amountToPay?: number): void {
    ...
    if (!this.movementOfCash.commissionAmount)
      this.movementOfCash.commissionAmount = 0.0;
    ...
    const values = {
      ...
      amountToPay: parseFloat(
        this.roundNumber.transform(this.amountToPay).toFixed(2)
      ),
      amountPaid: parseFloat(
        this.roundNumber.transform(this.amountPaid).toFixed(2)
      ),
      ...
      percentageAdministrativeExpense: parseFloat(
        this.roundNumber
          .transform(this.percentageAdministrativeExpense, 3)
          .toFixed(3)
      ),
      ...
      commissionAmount: parseFloat(
        this.roundNumber
          .transform(this.movementOfCash.commissionAmount)
          .toFixed(2)
      ),
      ...
      otherExpenseAmount: parseFloat(
        this.roundNumber
          .transform(this.movementOfCash.otherExpenseAmount)
          .toFixed(2)
      ),
      ...
      balanceCanceled: parseFloat(
        this.roundNumber
          .transform(this.movementOfCash.balanceCanceled)
          .toFixed(2)
      ),
    };
  ...
  onValueChanged(fieldID?: any): void {
    ...
    if (!fieldID || typeof fieldID === 'string') {
      ...
      for (let type of this.paymentMethods) {
        if (
          type._id.toString() === this.movementOfCashForm.value.paymentMethod
        ) {
      ...
      this.movementOfCash.expirationDate =
        this.movementOfCashForm.value.expirationDate;
    }
  }
  ...
  calculateQuotas(
    ...
    this.movementOfCash.taxPercentage =
      this.movementOfCashForm.value.taxPercentage;
    ...
    switch (field) {
      case 'quota':
        ...
        for (let i = 0; i < this.quotas; i++) {
          ...
          switch (this.interestType) {
            case 'Interés Simple':
              mov.interestAmount = this.roundNumber.transform(
                (this.roundNumber.transform(amountToPayTemp) *
              ...
              mov.capital = this.roundNumber.transform(
                amountToPayTemp / this.quotas
              );
            ...
            case 'Francés':
              ...
              let factorQuota: number = Math.pow(
                1 + tasa,
                this.quotas + 1 - mov.quota
              );
          ...
          this.totalInterestAmount = this.roundNumber.transform(
            this.totalInterestAmount
          );
        ...
        this.calculateQuotas(
          'expirationDate',
          moment(
            moment(this.movementOfCash.expirationDate, 'YYYY-MM-DD').format(
              'YYYY-MM-DD'
            )
          )
        ...
        if (
          ...
          for (let i = 0; i < this.movementsOfCashesToFinance.length; i++) {
            ...
            if (mov.quota === movement.quota) {
              ...
              isEdit = true;
              mov.expirationDate = moment(newValue)
                .format('YYYY-MM-DD')
                .toString();
            } else {
              if (isEdit) {
  ...
  async getMovementOfCashesByTransaction(op?: string) {
    try {
      ...
      if (
        ...
        this.amountPaid = -(
          lastMovement.amountPaid *
          this.movementOfCashForm.value.quotationNative
        );
        for (let paymentMethod of this.paymentMethods) {
          if (
            paymentMethod.currency &&
            paymentMethod.currency._id.toString() ===
              this.currencyNative._id.toString()
          ) {
            this.paymentMethodSelected = paymentMethod;
          }
                    ...
                    if (
                      ...
                      if (
                        ...
                        for (let movement of this.movementsOfCashes) {
                          amountPaid += this.roundNumber.transform(
                            movement.amountPaid
                          );
                        }
                      }
  ...
  async openModal(op: string, movement: MovementOfCash) {
    ...
    switch (op) {
                ...
                if (
                  ...
                  if (movementsOfArticles && movementsOfArticles.length > 0) {
                    let movementOfArticle: MovementOfArticle =
                      await this.deleteMovementOfArticle(
                        movementsOfArticles[0]
                      );
      ...
      case 'list-movements-of-cashes':
        ...
        modalRef.componentInstance.transactionAmount =
        ...
        modalRef.result.then(async (result) => {
          if (
            result &&
            result.movementsOfCashes &&
            result.movementsOfCashes.length > 0
          ) {
            try {
  ...
  changeAmountToPay(): void {
    if (this.keyboard)
      this.keyboard.setInput(
        this.movementOfCashForm.value.amountToPay.toString()
      );
    this.updateAmounts('amountToPay');
  }
  ...
  updateAmounts(op?: string): void {
    ...
    if (op !== 'amountToPay' && this.transaction.totalPrice !== 0) {
      this.amountToPay =
        this.transactionAmount - this.amountPaid - this.amountDiscount;
      if (this.keyboard) this.keyboard.setInput('');
    }
    ...
    return new Promise(async (resolve) => {
      try {
        ...
        if (
          ...
          this.amountToPay >
            this.roundNumber.transform(
              (this.transaction.totalPrice * this.movementOfCash.discount) /
                100 +
                this.transaction.totalPrice
            ) &&
          !this.paymentMethodSelected.acceptReturned
        ...
        if (
          ...
          this.amountToPay - 0.01 >
            this.roundNumber.transform(
              (this.transaction.totalPrice * this.movementOfCash.surcharge) /
                100 +
                this.transaction.totalPrice
            ) &&
          !this.paymentMethodSelected.acceptReturned
        ...
        if (
          ...
          throw new Error(
            'Debe seleccionar los métodos de pago en cartera a utilizar.'
          );
        }
        ...
        if (this.paymentMethodSelected.allowToFinance) {
          ...
          if (
            ...
            for (let mov of this.movementsOfCashesToFinance) {
              amountTotal = this.roundNumber.transform(
                amountTotal + mov.amountPaid
              );
              if (!moment(mov.expirationDate).isValid()) {
                throw new Error(
                  'Debe ingresar fechas de vencimiento de pago válidas'
                );
              } else {
                if (
  ...
  async changePercentageCommission(commissionAmount: number = null) {
    ...
    this.daysCommission =
      moment(
        moment(this.movementOfCashForm.value.expirationDate).format(
          'YYYY-MM-DD'
        ),
        'YYYY-MM-DD'
    ...
    if (commissionAmount != null) {
      ...
      this.percentageCommission =
        this.movementOfCashForm.value.percentageCommission;
      this.movementOfCash.commissionAmount = this.roundNumber.transform(
        ((this.amountToPay * this.percentageCommission) / 100) *
          this.daysCommission
      );
    }
    if (this.movementOfCash.taxPercentage > 0)
  ...
  changePercentageAdministrativeExpense(
    administrativeExpenseAmount: number = null
  ) {
    if (administrativeExpenseAmount != null) {
      ...
      this.percentageAdministrativeExpense = this.roundNumber.transform(
        100 *
          (this.movementOfCash.administrativeExpenseAmount / this.amountToPay)
      );
    } else {
      this.percentageAdministrativeExpense =
  ...
  changePercentageOtherExpense(otherExpenseAmount: number = null) {
    if (otherExpenseAmount != null) {
      ...
      this.percentageOtherExpense =
        this.movementOfCashForm.value.percentageOtherExpense;
      ...
      try {
        ...
        for (let mov of this.movementsOfCashes) {
          ...
          if (!mov.expirationDate || !moment(mov.expirationDate).isValid()) {
            throw new Error(
              'Debe ingresar fecha de vencimiento de pago válida'
            );
          }
        }
        ...
        if (
          this.transaction.totalPrice !== 0 &&
          this.roundNumber.transform(paid) <= 0
        ) {
          throw new Error(
      ...
      if (this.movementOfCashForm.valid) {
        if (!this.fastPayment) {
          if (await this.isValidAmount()) {
            if (!this.paymentMethodSelected.allowToFinance) {
              ...
              this.movementOfCash.type =
                this.movementOfCashForm.value.paymentMethod;
              ...
              if (this.paymentMethodSelected.checkDetail) {
                this.movementOfCash.receiver =
                  this.movementOfCashForm.value.receiver;
                this.movementOfCash.number =
                  this.movementOfCashForm.value.number;
                this.movementOfCash.titular =
                  this.movementOfCashForm.value.titular;
                this.movementOfCash.bank = this.movementOfCashForm.value.bank;
                this.movementOfCash.CUIT = this.movementOfCashForm.value.CUIT;
          ...
          this.movementOfCash.discount = this.movementOfCash.type
            ...
            : 0;
          this.movementOfCash.surcharge =
            this.movementOfCash.type.surcharge || 0;
          this.movementOfCash.interestPercentage =
            this.movementOfCashForm.value.interestPercentage;
    ...
    return new Promise<boolean>((resolve, reject) => {
      try {
        if (
          ...
          throw new Error(
            'Debe seleccionar una empresa para este método de pago'
          );
        if (
          this.movementOfCash.type.isCurrentAccount &&
              ...
              (result) => {
                if (result && result.status === 200) {
                  ...
                  if (total > (this.transaction.company.creditLimit || 0)) {
                    throw new Error(
                      'La empresa supera el limite de crédito otorgado'
                    );
                  } else {
                    resolve(true);
                  }
    ...
    try {
      ...
      if (
        ...
        movementOfArticle.description =
          'Pago con ' + this.paymentMethodSelected.name;
      } else if (
        ...
        if (movementOfArticle.taxes && movementOfArticle.taxes.length !== 0) {
          for (let movTax of movementOfArticle.taxes) {
            ...
            for (let transactionTax of this.transaction.taxes) {
              if (
                movTax.tax._id.toString() === transactionTax.tax._id.toString()
              ) {
