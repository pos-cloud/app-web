...
export class TableComponent implements OnInit {
  ...
  public buildForm(): void {
    this.tableForm = this._fb.group({
      ...
      room: [this.table.room, [Validators.required]],
      chair: [this.table.chair, [Validators.required]],
      state: [this.table.state, []],
    });

    this.tableForm.valueChanges.subscribe((data) => this.onValueChanged(data));
    ...
    this._tableService.updateTable(this.table).subscribe(
      (result) => {
        if (!result.table) {
          ...
          this.loading = false;
          this.showMessage(
            'La mesa se ha actualizado con éxito.',
            'success',
            false
          );
        }
    ...
    this._tableService.saveTable(this.table).subscribe(
      (result) => {
        if (!result.table) {
          ...
          this.table = result.table;
          this.showMessage(
            'La mesa se ha añadido con éxito.',
            'success',
            false
          );
          this.table = new Table();
