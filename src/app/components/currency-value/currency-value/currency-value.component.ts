...
export class CurrencyValueComponent implements OnInit {
  ...
  public setValueForm(): void {
    if (!this.currencyValue._id) {
      this.currencyValue._id = '';
    }
    if (!this.currencyValue.name) {
      this.currencyValue.name = '';
    }
    if (!this.currencyValue.value) {
      this.currencyValue.value = 0;
    }
  ...
  public buildForm(): void {
    this.currencyValueForm = this._fb.group({
      _id: [this.currencyValue._id, []],
      name: [this.currencyValue.name, [Validators.required]],
      value: [this.currencyValue.value, [Validators.required]],
    });

    this.currencyValueForm.valueChanges.subscribe((data) =>
  ...
  public updateCurrencyValue() {
    ...
    this._currencyValueService
      .updateCurrencyValue(this.currencyValue)
      .subscribe(
        (result) => {
          if (!result.currencyValue) {
            this.loading = false;
            ...
            this.loading = false;
            this.showMessage(
              'El valor se ha actualizado con éxito.',
              'success',
              false
            );
          }
    ...
    this._currencyValueService.saveCurrencyValue(this.currencyValue).subscribe(
      (result) => {
        if (!result.currencyValue) {
          ...
          this.loading = false;
          this.showMessage(
            'El valor se ha añadido con éxito.',
            'success',
            false
          );
          this.currencyValue = new CurrencyValue();
  ...
  public deleteCurrencyValue() {
    ...
    this._currencyValueService
      .deleteCurrencyValue(this.currencyValue._id)
      .subscribe(
        (result) => {
          this.loading = false;
          if (!result.currencyValue) {
