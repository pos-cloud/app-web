...
function get(database, transactionId) {
  return new Promise((resolve, reject) => {
    ...
    Transaction.findById(transactionId, (err, transaction) => {
      ...
      .populate({
        path: 'canceledTransactions.typeId',
        model: TransactionType,
      });
  });
}
...
function getTransaction(req, res, next, id = undefined) {
  ...
  get(req.session.database, transactionId)
    .then((transaction) => {
...
async function saveTransaction(req, res, next) {
  try {
    ...
    if (!transaction.number)
      transaction.number = await getLastNumberTransactions(
        req.session.database,
        transactionType._id,
        transaction.origin,
        transaction.letter
      );
    if (transactionType.maxOrderNumber > 0)
      transaction.orderNumber = await getLastOrderNumberTransactions(req.session.database, transactionType._id);
...
function setOrderNumberTransactions(req, transaction) {
  return new Promise((resolve, reject) => {
    ...
    axios
      .post(
        `${config.BASE_URL_V8}/set-order-number`,
        { transaction },
...
async function updateTransaction(req, res, next) {
  try {
    ...
    if (
      ...
      (transaction.CAE || transaction.SATStamp || transaction.stringSAT || transaction.CFDStamp)
    )
    ...
    if (oldTransaction.state === 'Entregado' && transaction.state === 'Armando')
    ...
    Transaction.find(where)
      ...
      .exec(async (err, transactions) => {
        ...
        if (!transactions || transactions.length === 0) {
          ...
          if (
            (transaction.type && transaction.type.electronics) ||
            transaction.state === 'Anulado' ||
...
function clearLookups(transaction) {
  if (transaction.currency && transaction.currency._id && transaction.currency._id !== '')
    transaction.currency = transaction.currency._id;
  if (transaction.relationType && transaction.relationType._id && transaction.relationType._id !== '')
    transaction.relationType = transaction.relationType._id;
  if (transaction.useOfCFDI && transaction.useOfCFDI._id && transaction.useOfCFDI._id !== '')
    transaction.useOfCFDI = transaction.useOfCFDI._id;
  if (transaction.type && transaction.type._id && transaction.type._id !== '') transaction.type = transaction.type._id;
  if (transaction.cashBox && transaction.cashBox._id && transaction.cashBox._id !== '')
    transaction.cashBox = transaction.cashBox._id;
  if (transaction.table && transaction.table._id && transaction.table._id !== '')
    transaction.table = transaction.table._id;
  if (transaction.employeeOpening && transaction.employeeOpening._id && transaction.employeeOpening._id !== '')
    transaction.employeeOpening = transaction.employeeOpening._id;
  if (transaction.employeeClosing && transaction.employeeClosing._id && transaction.employeeClosing._id !== '')
    transaction.employeeClosing = transaction.employeeClosing._id;
  if (transaction.branchOrigin && transaction.branchOrigin._id && transaction.branchOrigin._id !== '')
    transaction.branchOrigin = transaction.branchOrigin._id;
  if (transaction.branchDestination && transaction.branchDestination._id && transaction.branchDestination._id !== '')
    transaction.branchDestination = transaction.branchDestination._id;
  if (transaction.depositOrigin && transaction.depositOrigin._id && transaction.depositOrigin._id !== '')
    transaction.depositOrigin = transaction.depositOrigin._id;
  if (transaction.depositDestination && transaction.depositDestination._id && transaction.depositDestination._id !== '')
    transaction.depositDestination = transaction.depositDestination._id;
  if (transaction.company && transaction.company._id && transaction.company._id !== '')
    transaction.company = transaction.company._id;
  if (transaction.transport && transaction.transport._id && transaction.transport._id !== '')
    transaction.transport = transaction.transport._id;
  if (transaction.turnOpening && transaction.turnOpening._id && transaction.turnOpening._id !== '')
    transaction.turnOpening = transaction.turnOpening._id;
  if (transaction.turnClosing && transaction.turnClosing._id && transaction.turnClosing._id !== '')
    transaction.turnClosing = transaction.turnClosing._id;
  if (transaction.priceList && transaction.priceList._id && transaction.priceList._id !== '')
    transaction.priceList = transaction.priceList._id;
  if (transaction.creationUser && transaction.creationUser._id && transaction.creationUser._id !== '')
    transaction.creationUser = transaction.creationUser._id;
  if (transaction.updateUser && transaction.updateUser._id && transaction.updateUser._id !== '')
    transaction.updateUser = transaction.updateUser._id;
  return transaction;
}
...
async function deleteTransaction(req, res, next) {
  ...
  if (transactionId && transaction !== 'undefined') {
    await get(req.session.database, transactionId)
      .then((result) => {
        if (result) {
          transaction = result;
          isValid = true;
  ...
  if (isValid) {
    ...
    if (isValid) {
      axios.delete(`${config.BASE_URL_V8}/account-seats/transaction/${transaction._id}`, {
        headers: {
    ...
    if (isValid) {
      await returnBalanceMovementsOfCashesByCancelingTransaction(req, res, next, transactionId)
        .then(async (result) => {
          if (result) {
            isValid = true;
          } else {
    ...
    if (isValid) {
      await getTransactionsOrigin(req, res, next, transaction).then(async (result) => {
        if (result) {
...
async function returnBalanceMovementsOfCashesByCancelingTransaction(req, res, next, transactionId) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (isValid) {
      MovementOfCash.find(where).exec(async (err, movementsOfCashes) => {
        if (err) {
          reject(err);
        } else {
          for (const movementOfCash of movementsOfCashes) {
...
function getShiftClosing(req, res, next) {
  ...
  Transaction.find(where).exec((err, transactions) => {
    if (err) {
      fileController.writeLog(req, res, next, 500, err);
      return res.status(500).send(constants.ERR_SERVER);
    } else {
      ...
      if (movementsOfCancellations && movementsOfCancellations.length > 0) {
        for (const mov of movementsOfCancellations) {
          ...
          await updateTransactionById(req, res, next, mov.transactionOrigin)
            .then((transaction) => {
              if (
                (mov.transactionOrigin.type.transactionMovement === 'Venta' &&
      ...
      if (err) {
        ...
        if (transactions && transactions.length > 0) {
          ...
          await Transaction.findByIdAndUpdate(
            transactions[0]._id,
            clearLookups(transactions[0]),
            (err, transactionUpdated) => {
              if (err) {
                reject(err);
              } else {
...
function getTransactionsOrigin(req, res, next, transactionDestination) {
  ...
  return new Promise((resolve, reject) => {
    ...
    MovementOfCancellation.find(where)
      .populate({
