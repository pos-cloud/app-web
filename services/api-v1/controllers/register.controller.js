...
async function register(req, res, next) {
	try {
		...
		let identificationTypes = [{ name: 'CUIT' }, { name: 'DNI' }, { name: 'Pasaporte' }, { name: 'Otro' }];
		code = 0;
		...
		for (let i of identificationTypes) {
			...
			if (i.name === 'DNI') identificationTypeDNI = result;
			code++;
		}

		let companies = [
			{ name: 'Consumidor final', type: 'Cliente' },
			{ name: 'Proveedor Prueba', type: 'Proveedor' }
		];
		...
		let paymentMethods = [
			...
			{ code: 2, name: 'Cuenta Corrient', isCurrentAccount: true, acceptReturned: false, inputAndOuput: false, checkDetail: false, cardDetail: false, allowToFinance: false, cashBoxImpact: false },
			...
			{ code: 6, name: 'Préstamos', isCurrentAccount: true, acceptReturned: false, inputAndOuput: false, checkDetail: true, cardDetail: false, allowToFinance: true, cashBoxImpact: false },
		];
