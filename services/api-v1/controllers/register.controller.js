...
async function register(req, res, next) {
  try {
    ...
    if (!companyName)
      throw {
        status: 400,
        message: 'Debe ingresar el nombre del negocio (companyName).',
      };

    if (!password)
      throw { status: 400, message: 'Debe ingresar la contraseña (password).' };

    if (!category) {
      throw {
        status: 400,
        message: 'Debe ingresar la categoría del negocio (category).',
      };
    } else {
    ...
    if (!email)
      throw {
        status: 400,
        message: 'Debe ingresar el email del usuario (email)',
      };

    if (!validateEmail(email))
      throw {
        status: 400,
        message: 'El email " + email + " es incorrecto (email)',
      };
    ...
    let users = [
      ...
      {
        name: 'soporte',
        email: 'info@poscloud.com.ar',
        employee: employeeSoporte,
    ...
    let companies = [{ name: 'Consumidor final', type: 'Cliente' }];

    code = 1;
    let getIdentificationTypeDNI = await IdentificationType.find({
      name: 'DNI',
    });
    ...
    let transactionTypes = [
      // VENTA
      {
        ...
        name: 'Ticket',
        currentAccount: 'No',
        requestCompany: 'Cliente',
        entryAmount: 'Venta con IVA',
        fixedLetter: 'X',
        movement: 'Entrada',
        modifyStock: false,
        stockMovement: null,
      ...
      // COMPRA
      {
        ...
        name: 'Factura',
        currentAccount: 'Si',
        requestCompany: 'Proveedor',
        entryAmount: 'Costo sin IVA',
        fixedLetter: '',
        movement: 'Salida',
        modifyStock: true,
        stockMovement: 'Entrada',
    ...
    return res
      .status(200)
      .send({ password: passwordSoporte, user: userSoporte.name, db: dbName });
  } catch (error) {
...
function existsDatabase(database) {
  return new Promise((resolve, reject) => {
    try {
      ...
      Database.find(where).exec((err, databases) => {
        if (err) throw err;
        else if (databases && databases.length > 0)
          reject({
            status: 409,
            message: 'El nombre del negocio ya se encuentra registrado.',
          });
        resolve();
      });
    } catch (err) {
...
function saveUser(user) {
  return new Promise((resolve, reject) => {
    ...
    bcryptjs
      .hash(user.password, bcryptjs_SALT_ROUNDS)
