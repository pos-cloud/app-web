    ...
    if (mongoose.Types.ObjectId.isValid(id)) {
      await MovementOfArticle.findById(id, (err, movementOfArticle) => {
        ...
        .populate({
          ...
          model: Article,
          populate: [
            { path: 'unitOfMeasurement', model: UnitOfMeasurement },
        ...
        .populate({
          ...
          model: Transaction,
          populate: [
            { path: 'type', model: TransactionType },
            { path: 'taxes.tax', model: Tax },
          ],
        })
...
async function update(req, res, next, movementOfArticleId, movementOfArticle) {
  return new Promise(async (resolve, reject) => {
    ...
    await MovementOfArticle.updateOne(
      { _id: movementOfArticleId },
      clearLookups(movementOfArticle),
      async (err, result) => {
        if (err) {
...
async function save(req, res, next, movementOfArticle) {
  return new Promise(async (resolve, reject) => {
    ...
    movementOfArticle.markupPriceWithoutVAT = roundNumber(
      movementOfArticle.salePrice - totalTaxes - movementOfArticle.basePrice
    );
    ...
    await movementOfArticle.save(async (err, movementOfArticleSaved) => {
      if (err) {
        ...
        if (movementOfArticleSaved) {
          await get(movementOfArticleSaved._id)
            .then((movementOfArticle) => {
              resolve(movementOfArticle);
            })
            .catch((err) => {
...
function getMovementsOfArticlesV2(req, res, next) {
  ...
  if (
    req.query &&
    req.query !== {} &&
    (!req.query.fullQuery || req.query.fullQuery === '[]' || req.query.fullQuery === [])
  ) {
    let error;
    ...
    if (project && project !== {} && project !== '{}') {
      try {
        ...
        if (searchPropertyOfArray(project, 'account.')) {
          queryAggregate.push({
            $lookup: { from: 'accounts', foreignField: '_id', localField: 'account', as: 'account' },
          });
        ...
        if (searchPropertyOfArray(project, 'deposit.')) {
          queryAggregate.push({
            $lookup: { from: 'deposits', foreignField: '_id', localField: 'deposit', as: 'deposit' },
          });
          queryAggregate.push({ $unwind: { path: '$deposit', preserveNullAndEmptyArrays: true } });

          queryAggregate.push({
            $lookup: { from: 'branches', foreignField: '_id', localField: 'deposit.branch', as: 'deposit.branch' },
          });
        ...
        if (searchPropertyOfArray(project, 'article.')) {
          queryAggregate.push({
            $lookup: { from: 'articles', foreignField: '_id', localField: 'article', as: 'article' },
          });
          queryAggregate.push({ $unwind: { path: '$article', preserveNullAndEmptyArrays: true } });

          if (searchPropertyOfArray(project, 'article.deposit.')) {
            queryAggregate.push({
              $lookup: { from: 'deposits', foreignField: '_id', localField: 'article.deposit', as: 'article.deposit' },
            });
          ...
          if (
            searchPropertyOfArray(project, 'article.otherFields.') ||
            searchPropertyOfArray(project, 'otherFieldsValues')
          ) {
            queryAggregate.push({
              $lookup: { from: 'companies', foreignField: '_id', localField: 'providers', as: 'providers' },
            });
          }
          if (
            searchPropertyOfArray(project, 'article.otherFields.') ||
            ...
            queryAggregate.push({
              $lookup: { from: 'companies', foreignField: '_id', localField: 'provider', as: 'provider' },
            });
          }
          ...
          if (searchPropertyOfArray(project, 'articleParent')) {
            ...
            queryAggregate.push({
              $lookup: { from: 'articles', foreignField: '_id', localField: 'articleParent._id', as: 'articleParent' },
            });
            queryAggregate.push({ $unwind: { path: '$articleParent', preserveNullAndEmptyArrays: true } });
          }
        }
          ...
          if (searchPropertyOfArray(project, 'transaction.employeeOpening.')) {
            queryAggregate.push({
              $lookup: {
                from: 'employees',
                foreignField: '_id',
                localField: 'transaction.employeeOpening',
                as: 'transaction.employeeOpening',
              },
            });
            queryAggregate.push({
              $unwind: { path: '$transaction.employeeOpening', preserveNullAndEmptyArrays: true },
            });
          }
          ...
          if (searchPropertyOfArray(project, 'transaction.employeeClosing.')) {
            queryAggregate.push({
              $lookup: {
                from: 'employees',
                foreignField: '_id',
                localField: 'transaction.employeeClosing',
                as: 'transaction.employeeClosing',
              },
            });
            queryAggregate.push({
              $unwind: { path: '$transaction.employeeClosing', preserveNullAndEmptyArrays: true },
            });
          }
        ...
        if (searchPropertyOfArray(project, 'creationUser.')) {
          queryAggregate.push({
            $lookup: { from: 'users', foreignField: '_id', localField: 'creationUser', as: 'creationUser' },
          });
        ...
        if (searchPropertyOfArray(project, 'updateUser.')) {
          queryAggregate.push({
            $lookup: { from: 'users', foreignField: '_id', localField: 'updateUser', as: 'updateUser' },
          });
...
async function saveMovementOfArticle(req, res, next) {
  ...
  await save(req, res, next, movementOfArticle)
    .then(async (movementOfArticle) => {
...
async function updateMovementOfArticle(req, res, next) {
  ...
  await update(req, res, next, movementOfArticleId, movementOfArticle)
    .then((movementOfArticle) => {
      fileController.writeLog(req, res, next, 200, 'ok');
      return res.status(200).send({ movementOfArticle });
    })
...
async function deleteMovementOfArticle(req, res, next) {
  ...
  await updateStockByDelete(req, res, next, where)
    .then((result) => {
      const user = new User();
      user._id = req.session.user;
      ...
      MovementOfArticle.findByIdAndUpdate(movementOfArticleId, set, (err, movementOfArticleDeleted) => {
        if (err) {
          fileController.writeLog(req, res, next, 500, err);
          return res.status(500).send(constants.ERR_SERVER);
        } else {
...
async function updateStockByDelete(req, res, next, query = undefined) {
  return new Promise(async (resolve, reject) => {
    ...
    await MovementOfArticle.find(query).exec(async (err, movementsOfArticles) => {
      if (err) {
        reject(err);
      } else {
        if (!movementsOfArticles || movementsOfArticles.length === 0) {
          ...
          for (const mov of movementsOfArticles) {
            if (mov.quantityForStock !== 0) {
              await updateArticleStock(req, res, next, mov)
                .then(async (result) => {
                  mov.quantityForStock = 0;
                  await update(req, res, next, mov._id, mov)
                    .then((movementOfArticle) => {})
                    .catch((err) => {
                      reject(err);
                    });
    ...
    try {
      ...
      await ArticleStock.find(where).exec(async (err, articleStocks) => {
        if (err) {
          ...
          await ArticleStockController.update(req, res, next, articleStocks[0]._id, articleStocks[0])
            .then((articleStock) => {
...
function clearLookups(movementOfArticle) {
  if (
    ...
    movementOfArticle.movementParent._id &&
    movementOfArticle.movementParent._id !== ''
  )
    movementOfArticle.movementParent = movementOfArticle.movementParent._id;
  if (movementOfArticle.make && movementOfArticle.make._id && movementOfArticle.make._id !== '')
    movementOfArticle.make = movementOfArticle.make._id;
  if (movementOfArticle.category && movementOfArticle.category._id && movementOfArticle.category._id !== '')
    movementOfArticle.category = movementOfArticle.category._id;
  if (movementOfArticle.deposit && movementOfArticle.deposit._id && movementOfArticle.deposit._id !== '')
    movementOfArticle.deposit = movementOfArticle.deposit._id;
  if (movementOfArticle.article && movementOfArticle.article._id && movementOfArticle.article._id !== '')
    movementOfArticle.article = movementOfArticle.article._id;
  if (movementOfArticle.transaction && movementOfArticle.transaction._id && movementOfArticle.transaction._id !== '')
    movementOfArticle.transaction = movementOfArticle.transaction._id;
  if (movementOfArticle.creationUser && movementOfArticle.creationUser._id && movementOfArticle.creationUser._id !== '')
    movementOfArticle.creationUser = movementOfArticle.creationUser._id;
  if (movementOfArticle.updateUser && movementOfArticle.updateUser._id && movementOfArticle.updateUser._id !== '')
    movementOfArticle.updateUser = movementOfArticle.updateUser._id;
