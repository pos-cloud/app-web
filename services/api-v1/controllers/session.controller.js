...
function getSession(req, res, next, id = undefined) {
...
function getSessions(req, res, next) {
		...
		if (limit) {
			try {
				...
				if (limit > 0) {
					...
					if (group && group !== "{}" && group !== {}) {
						let projectGroup;
						if (searchPropertyOfArray(JSON.parse(group), 'sessions')) {
							projectGroup = `{ "sessions": { "$slice": ["$sessions", ${skip}, ${limit}] }`;
						} else {
							projectGroup = `{ "items": { "$slice": ["$items", ${skip}, ${limit}] }`;
						}
						for (let prop of Object.keys(JSON.parse(group))) {
							if (prop !== 'sessions' && prop !== 'items') {
								projectGroup += `, "${prop}": 1`;
							}
						}
						projectGroup += `}`;
	...
	Session.aggregate(queryAggregate)
		.then(function (result) {
			...
			if (result.length > 0) {
				if (group && group !== "{}" && group !== {}) {
					...
					return res.status(200).send({ sessions: result });
				}
			} else {
				if (group && group !== "{}" && group !== {}) {
					return res.status(200).send({ count: 0, sessions: [] });
				} else {
...
function saveSession(req, res, next, params) {
	...
	session.date = moment().format('YYYY-MM-DDTHH:mm:ssZ');
	session.state = params.state;
	session.user = params.user;
	session.userName = params.userName;
	session.token = params.token;
	session.tokenExpiration = params.tokenExpiration;
...
function initConnectionDB(database) {
	let SessionSchema = require('./../models/session');
