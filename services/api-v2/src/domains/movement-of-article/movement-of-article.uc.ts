...
export default class MovementOfArticleUC {
  ...
  constructor(database: string) {
    ...
    this.movementOfArticleController = new MovementOfArticleController(
      database,
    );
  ...
  createMovementOfArticle = async (
    ...
    let movementOfArticle: MovementOfArticle =
      MovementOfArticleSchema.getInstance(this.database);
    let config: Config;
    ...
    movementOfArticle.deposit = deposit
      ? deposit
      : transaction.depositDestination;
      ...
      if (transaction.type.requestTaxes) {
        ...
        if (article.taxes && article.taxes.length > 0) {
          ...
          for (let taxAux of article.taxes) {
            ...
            if (taxAux.percentage === 0)
              taxAmount = roundNumber(
                taxAux.taxAmount * movementOfArticle.amount,
                4,
              );
            else
              taxAmount = roundNumber((taxBase * taxAux.percentage) / 100, 4);
    ...
    await new TransactionUC(this.database).updateByMovementOfArticle(
      movementOfArticle,
    );
  ...
  deleteByTransaction = async (transactionId: string): Promise<boolean> => {
    return new Promise<boolean>(async (resolve, reject) => {
      try {
        ...
        movementsOfArticles.forEach(
          async (movementOfArticle: MovementOfArticle) => {
  ...
  deleteMovementOfArticle = async (
    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        ...
        let articleStockController: ArticleStockController =
        ...
        if (movementOfArticle.deposit) {
          await articleStockController.updateMany(
            {
              article: movementOfArticle.article,
              deposit: movementOfArticle.deposit,
            },
    ...
    return new Promise<boolean>(async (resolve, reject) => {
      try {
        ...
        if (movementsOfArticles && movementsOfArticles.length > 0) {
          let transactionOriginId =
            movementsOfArticles[0].movementOrigin.transaction._id;

          // update mov origin
          for (const mov of movementsOfArticles) {
            // actualizamos las lecturas en el origen
            await this.movementOfArticleController.update(
              mov.movementOrigin._id,
              {
          ...
          const movArticleOrigin =
            await this.movementOfArticleController.getAll({
              project: {
                _id: 1,
                "transaction._id": 1,
                amount: 1,
